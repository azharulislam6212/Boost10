/**
 * boost-carousel.js — Boost10
 *
 * `<boost-carousel>` — the theme's own carousel engine. No Swiper, no third
 * party anything.
 *
 * ## What it is
 *
 ...
 * That is the single biggest difference from the Swiper adapter it replaces:
 * Swiper measured the track and then wrote an inline `width` onto every slide,
 * so the layout existed twice — once in `carousel-vars`, once in Swiper's
 * arithmetic — and the slides visibly resized on the frame Swiper mounted.
 * Here the CSS that sizes a slide before the module loads is the same CSS that
 * sizes it afterwards, so there is no mount jump to hide and no `spaceBetween`
 * to keep in sync with a `gap`.
 *
 * ## Markup
 *
 *   <boost-carousel
 ...
 *   </boost-carousel>
 *
 * That is the shape `snippets/carousel-shell.liquid` already renders, refs and
 * all, so migrating a section is a tag rename rather than a rewrite. Controls
 * rendered outside the element link back with `data-carousel-for="<id>"`,
 ...
 *
 *   <boost-carousel data-options='{"loop":true,"autoplay":{"delay":5000}}'>
 *
...
export class BoostCarousel extends BaseComponent {
  ...
  get options() {
    ...
    if (raw) {
      try {
        ...
        console.error('[Boost10] <boost-carousel> could not parse data-options.', error, raw);
      }
    }
   ...
   * neither attribute — the plain `<boost-carousel>` of the brief — runs.
   *
   * @returns {boolean}
   */
  get #shouldRun() {
  ...
  #start() {
    ...
    // first slide, which is what the Swiper adapter's `#init()` guarded against
    // with its `if (this.#swiper && !this.#swiper.destroyed) return`. Dragging a
    // window across 990px is not an instruction to rewind the carousel.
    if (this.running) {
      this.#refit();
      return;
    }
    ...
    // such wrapper — it is the control. `base.css` can only reveal a nested one,
   ...
   * the Swiper adapter repeated in JavaScript: below twice the widest
   * slides-per-view there are not enough slides to fill the viewport on the far
   ...
   *
   * `--carousel-slides` is stepped per breakpoint by `boost-carousel.css` from
   ...
   * makes it impossible for the layout and the arithmetic to disagree — which
   * is the whole class of bug the Swiper adapter spent `#gap()` and `#syncGap()`
   * defending against.
   *
   * @returns {number}
   */
  ...
  #update({ changed = true } = {}) {
    ...
    // Swiper reported `progress: 0` forever when the slides already fit, and an
    // empty bar in that state reads as broken. Full is the honest answer:
   ...
   *
   * Swiper rebuilt its bullets on every `slideChange`, which is why the dots
   * flickered on a fast drag. Here the elements are made once and only an
   * attribute moves afterwards.
   */
  #renderPagination() {
    ...
    if (type === 'fraction') {
      ...
      current.className = 'boost-carousel__fraction-current';
      const separator = document.createTextNode(' / ');
      const count = document.createElement('span');
      count.className = 'boost-carousel__fraction-total';
      element.append(current, separator, count);
      return;
    }

    const fill = document.createElement('span');
    fill.className = 'boost-carousel__progress-fill';
  ...
  #updatePagination(pageIndex, pageCount, progress) {
    ...
    if (type === 'fraction') {
      const current = element.querySelector('.boost-carousel__fraction-current');
      const count = element.querySelector('.boost-carousel__fraction-total');
    ...
    const fill = /** @type {HTMLElement|null} */ (element.querySelector('.boost-carousel__progress-fill'));
    if (!fill) return;

    // Slide-based rather than translate-based, which is what Swiper's
    // progressbar type did: the bar is a position readout beside a row of
    // slides, not a scrollbar. The `.carousel-controls__bar` beside the numbers
    // keeps using the translate progress, as it always has.
    //
    // Always `scaleX`, including for a vertical carousel. Swiper turned its
    // progressbar with the carousel and this theme spends several rules in
    // `base.css` turning it back — a vertical carousel's pagination is drawn
    // under it, horizontally, because the element clips to its own height and a
    // bar standing up the side of it has nowhere to be. The bar's own box
    // decides its axis, not the track's.
   ...
   * starts, and `boost-carousel.css` holds an armed slide at its resting state
   ...
   * clone work — the two mistakes that made the equivalent observer on the
   * Swiper adapter re-enter itself on every loop wrap.
   */
  #watchSlides() {
    const track = this.track;
    if (!track) return;
  ...
  #playAutoplay() {
    ...
    // wrong state. This is what the Swiper adapter did too, by way of
    // `config.autoplay = false` and `pause.hidden = !this.#hasAutoplay`.
   ...
   *
   * `--carousel-autoplay-progress` is the property `base.css` already builds
   * the ring's conic gradient from. The Swiper adapter wrote it once per frame
   * from `autoplayTimeLeft`; here it is a CSS transition from 0 to 1 over the
   * autoplay delay, which is the same sweep with no JavaScript running during
   * it. `boost-carousel.css` registers the property with `@property` so it can
   * be interpolated — without that registration it is a string and the ring
   * simply steps, which is the graceful degradation.
   *
   ...
   * keeps `<boost-carousel data-options='{"slidesPerView":3}'>` a complete
...
defineComponent('boost-carousel', BoostCarousel);
